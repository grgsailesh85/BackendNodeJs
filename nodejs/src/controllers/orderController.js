...
const createOrder = async (req, res) => {
  ...
  if (!input.orderItems || input.orderItems?.length == 0)
    return res.status(422).send("Order items are require.");
...
const updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    if (!input.status)
      return res.status(422).send("Orders status is required.");
...
const confirmOrder = async (req, res) => {
  ...
  try {
    if(!input.status) return res.status(422).send("Order confirm status is required")
